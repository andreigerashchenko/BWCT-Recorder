...
night_cam = None
night_cfg = None
use_night = True # True = night, False = day
...
def update_frame():
    ...
    global PREVIEW_FRAMERATE, use_night, day_cam, night_cam, latest_frame_queue, latest_frame, video_writer
    frame = None
    last_frame_time = 0
    while True:
        data = io.BytesIO()
        if use_night:
            ...
            if video_writer is not None and latest_frame is not None:
                video_writer.write(cv2.cvtColor(latest_frame, cv2.COLOR_RGB2BGR))
        frame = data.getvalue()
...
def camera_worker(preview_framerate, queue, state):
    ...
    global RESOLUTION, PREVIEW_FRAMERATE, day_cam, day_cfg, latest_frame_queue, encoder, night_cam, night_cfg, latest_frame, use_night, last_camera_check, video_writer
    ...
    # day_cfg = day_cam.create_video_configuration(main={"size": RESOLUTION})
    ...
    while True:
        ...
        #         output_file = f"{state['recording_directory']}/video_{segment_count}.mp4"
        #         # output = FileOutput(f"{state['recording_directory']}/video_{segment_count}.mp4")
        #         # day_cam.start_encoder(encoder, output, quality=Quality.HIGH)
        #         video_writer = cv2.VideoWriter(output_file, cv2.VideoWriter_fourcc(*'mp4v'), PREVIEW_FRAMERATE, RESOLUTION)
        ...
        if should_combine:
            ...
            time.sleep(5) # Wait a few seconds for the last segment to write
